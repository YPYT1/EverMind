...
儲存路徑：`~/.evermind/catalog.db`。專案與工作區 ID 用於記錄來源及調整排序，不限制記憶讀取。
...
腳本會檢查 Python 3.12+，在缺少時安裝 uv，同步依賴，並自動配置 Claude Desktop 和 Cursor。
...
系統預設使用內建的 `intfloat/multilingual-e5-small` 模型提供離線中英文語義檢索。設定外部 embedding 或 rerank API 後，服務健康時優先使用外部能力，失敗時自動回退到本地模型。

## MCP 工具

EverMind 透過同一個 `evermind` MCP 伺服器暴露 50 個工具：14 個記憶工具、13 個程式碼圖譜工具、20 個本地 Basic Memory 工具、2 個審核歸檔更新工具和 1 個統一專案生命週期工具。不需要安裝外部 Basic Memory CLI 或 codebase-memory 二進位檔。

| 工具 | 說明 |
|------|------|
| `remember(content, importance, tags)` | 儲存記憶。importance: 0=工作(24h), 1=長期, 2=永久 |
| `update_memory(id, content, tags, meta)` | 建立修正版並標記舊版本已被取代；同步更新衍生索引 |
