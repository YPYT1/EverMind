...
存储路径：`~/.evermind/catalog.db`。项目和工作区 ID 用于记录来源与调整排序，不限制记忆读取。

## EverMind 的工作方式

EverMind 由两个协同工作的组件组成：

**MCP 服务器** — Claude Code 直接调用的工具：
...
- `skills/evermind/SKILL.md` — 核心工作流：会话启动协议、何时保存记忆
- `skills/evermind-archive/SKILL.md` — 永久知识存储模式
- `skills/evermind-code-graph/SKILL.md` — 代码库探索
- `skills/project-memory/SKILL.md` — 首次项目初始化

在你的 `CLAUDE.md` 或 `AGENTS.md` 中引用技能：

```markdown
$evermind
```

> **两个组件都需要。** MCP 服务器赋予 Claude 记忆能力，Skills 告诉 Claude 何时使用这个能力。
...
脚本会检查 Python 3.12+，在缺少时安装 uv，同步依赖，并自动配置 Claude Desktop 和 Cursor。
...
系统默认使用内置的 `intfloat/multilingual-e5-small` 模型提供离线中英文语义检索。配置外部 embedding 或 rerank API 后，服务健康时优先使用外部能力，失败时自动回退到本地模型。

## MCP 工具

EverMind 通过同一个 `evermind` MCP 服务器暴露 50 个工具：14 个记忆工具、13 个代码图谱工具、20 个本地 Basic Memory 工具、2 个审核归档更新工具和 1 个统一项目生命周期工具。不需要安装外部 Basic Memory CLI 或 codebase-memory 二进制。

| 工具 | 说明 |
|------|------|
| `remember(content, importance, tags)` | 保存记忆。importance: 0=工作(24h), 1=长期, 2=永久 |
| `update_memory(id, content, tags, meta)` | 创建修正版本并标记旧版本已被取代；同步更新派生索引 |
