...
bool cbm_pipeline_file_has_error(const cbm_pipeline_t *p, const char *rel_path) {
    if (!p || !rel_path) {
        return false;
    }
    for (int i = 0; i < p->file_errors_count; i++) {
        if (p->file_errors[i].path && strcmp(p->file_errors[i].path, rel_path) == 0) {
            return true;
        }
    }
    return false;
...
static int dump_and_persist_hashes(cbm_pipeline_t *p, const cbm_file_info_t *files, int file_count,
    ...
    if (hash_store) {
        ...
        if (fhashes) {
            int fh_n = 0;
            for (int i = 0; i < file_count; i++) {
                if (cbm_pipeline_file_has_error(p, files[i].rel_path)) {
                    continue;
                }
                struct stat fst;
