...
static bool preserve_missing_path(const char *repo_path, const char *rel_path) {
    char abs_path[CBM_SZ_4K];
    int n = snprintf(abs_path, sizeof(abs_path), "%s/%s", repo_path, rel_path);
    if (n < 0 || n >= (int)sizeof(abs_path)) {
        cbm_log_warn("incremental.path_truncated", "rel_path", rel_path);
        return true;
    }
    struct stat st;
    ...
    if (errno != ENOENT && errno != ENOTDIR) {
        cbm_log_warn("incremental.stat_uncertain", "rel_path", rel_path, "errno",
                     itoa_buf(errno));
        return true;
    }
    return false;
...
static int find_deleted_files(const char *repo_path, cbm_file_info_t *files, int file_count,
                              ...
                              const cbm_coverage_row_t *coverage, int coverage_count,
                              char ***out_deleted, cbm_file_hash_t **out_mode_skipped,
    ...
    *out_mode_skipped_count = 0;
    ...
    CBMHashTable *classified = cbm_ht_create(
        (size_t)(stored_count + coverage_count > 0 ? stored_count + coverage_count : CBM_SZ_64) *
        PAIR_LEN);
    for (int i = 0; i < file_count; i++) {
    ...
    for (int i = 0; i < stored_count; i++) {
        cbm_ht_set(classified, stored[i].rel_path, &stored[i]);
        ...
        bool preserve = preserve_missing_path(repo_path, stored[i].rel_path);
    ...
    /* Failed and partial files intentionally have no hash so a normal reindex
     * retries them. Their persisted coverage rows are therefore the only
     * durable evidence available when such a file is later deleted. */
    for (int i = 0; i < coverage_count; i++) {
        const char *rel_path = coverage[i].rel_path;
        bool by_design = coverage[i].kind && strncmp(coverage[i].kind, "not_indexed", 11) == 0;
        if (!rel_path || by_design || cbm_ht_get(current, rel_path) ||
            cbm_ht_get(classified, rel_path)) {
            continue;
        }
        cbm_ht_set(classified, rel_path, classified);
        if (preserve_missing_path(repo_path, rel_path)) {
            continue;
        }
        if (del_count >= del_cap) {
            del_cap *= PAIR_LEN;
        ...
        deleted[del_count++] = strdup(rel_path);
    }
...
static void persist_hashes(cbm_pipeline_t *pipeline, cbm_store_t *store, const char *project,
    ...
    for (int i = 0; i < file_count; i++) {
        if (cbm_pipeline_file_has_error(pipeline, files[i].rel_path)) {
            continue;
        }
        struct stat st;
        if (stat(files[i].path, &st) != 0) {
...
static void dump_and_persist(cbm_pipeline_t *pipeline, cbm_gbuf_t *gbuf, const char *db_path,
    ...
    if (hash_store) {
        persist_hashes(pipeline, hash_store, project, files, file_count, mode_skipped,
                       mode_skipped_count);

        /* Coverage rows (#963): re-write the authoritative merged set into
         * the rebuilt DB. */
        if (cov_count > 0 &&
...
int cbm_pipeline_run_incremental(cbm_pipeline_t *p, const char *db_path, cbm_file_info_t *files,
    ...
    /* Capture coverage before classification: failed and partial files have no
     * hash, so their coverage rows are needed to detect a later deletion. */
    ...
    int deleted_count =
        find_deleted_files(cbm_pipeline_repo_path(p), files, file_count, stored, stored_count,
                           old_cov, old_cov_count, &deleted, &mode_skipped, &mode_skipped_count);
     ...
     * Rows for deleted files are filtered against this run's discovered file
     * set. Borrowed strings: old_cov and the pipeline own them past the
    ...
    if (cov) {
        ...
        CBMHashTable *deleted_set =
            cbm_ht_create(deleted_count > 0 ? (size_t)deleted_count * PAIR_LEN : CBM_SZ_64);
        for (int i = 0; i < ci; i++) {
            cbm_ht_set(changed_set, changed_files[i].rel_path, &changed_files[i]);
        }
        for (int i = 0; i < deleted_count; i++) {
            cbm_ht_set(deleted_set, deleted[i], deleted[i]);
        }
        for (int i = 0; i < old_cov_count; i++) {
            ...
            if (!by_design && old_cov[i].rel_path &&
                !cbm_ht_get(deleted_set, old_cov[i].rel_path) &&
