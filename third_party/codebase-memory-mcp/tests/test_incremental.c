 ...
 *
 * Clones an exact offline snapshot of FastAPI 0.99.1, indexes it through the
 * full pipeline, then tests incremental re-indexing with file modifications,
 * additions, deletions, and adversarial inputs.
 *
 ...
 *
 * The checked-in Git bundle keeps the suite deterministic and offline.
 */
...
static int create_incremental_fixture(void) {
    static const char *bundle = "tests/fixtures/fastapi-0.99.1.bundle";
    static const char *expected_tree = "2a0c768e0e8dfd1b7093daf7c27b503480d3b22e";
    const char *clone_argv[] = {"git", "clone", "--quiet", "--branch", "fixture", "--",
                                bundle, g_repodir, NULL};
    if (cbm_exec_no_shell(clone_argv) != 0) {
        return -1;
    }
    const char *verify_argv[] = {"git", "-C", g_repodir, "diff", "--quiet", expected_tree,
                                 "HEAD", "--", NULL};
    return cbm_exec_no_shell(verify_argv);
}
...
static int incremental_setup(void) {
    ...
    if (create_incremental_fixture() != 0) {
        printf("  offline FastAPI fixture setup failed\n");
        return -1;
    }
...
TEST(incr_full_index) {
    ...
    size_t rss_before_mb = g_rss_before_full / (1024 * 1024);
    size_t peak_before_mb = g_peak_before_full / (1024 * 1024);
    size_t measured_peak_mb = peak_mb;
    if (peak_before_mb > rss_before_mb + 64) {
        /* The OS lifetime peak belongs to an earlier suite in the shared test
         * runner. Use the retained RSS for this phase; isolated incremental
         * runs still exercise the true peak path above. */
        measured_peak_mb = cbm_mem_rss() / (1024 * 1024);
    }
    size_t rss_delta_mb = measured_peak_mb > rss_before_mb ? measured_peak_mb - rss_before_mb : 0;
...
TEST(incremental_fixture_setup) {
    if (incremental_setup() != 0) {
        FAIL("offline FastAPI incremental fixture setup failed");
    }
    PASS();
}

SUITE(incremental) {
    RUN_TEST(incremental_fixture_setup);
    if (!g_srv) {
        return;
    }
