...
static cbm_store_t *ri_index_capture(RProj *lp, char **out_resp) {
    ...
    if (th_project_db_path(lp->dbpath, sizeof(lp->dbpath), lp->project) != 0) {
        return NULL;
    }
    unlink(lp->dbpath);
...
TEST(index_parse_partial_reported) {
    ...
    /* A deleted partial file must not leave stale coverage behind. Partial
     * files intentionally have no hash so they are retried; discovery is the
     * authoritative live-file set for cleanup. */
    cbm_store_close(store);
    store = NULL;
    char split_path[700];
    snprintf(split_path, sizeof(split_path), "%s/split.c", lp.tmpdir);
    unlink(split_path);
    char iargs[700];
    ...
    free(resp2);
    char *after_delete = cbm_mcp_handle_tool(lp.srv, "index_status", qargs);
    ASSERT_NOT_NULL(after_delete);
    ASSERT_NULL(strstr(after_delete, "split.c"));
    free(after_delete);
