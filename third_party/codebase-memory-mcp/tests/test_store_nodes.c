...
/* Authoritative round-trip + shadow miss-graph materialization + empty-replace
 * wipe. Failed and partial files intentionally have no hash so a later reindex
 * retries them; the pipeline filters deleted paths before this store call. */
TEST(store_coverage_roundtrip_shadow) {
    cbm_store_t *s = cbm_store_open_memory();
    cbm_store_upsert_project(s, "test", "/tmp/test");
    ...
    cbm_coverage_row_t rows[] = {
        ...
        /* By-design rows stay out of the shadow miss graph. */
    ...
    ASSERT_EQ(saw_failure, 2);
    ASSERT_EQ(saw_by_design, 2);
    cbm_store_free_coverage(got, n);

    /* Shadow miss-graph materialized under "test::missed": FAILURES only;
     * the by-design rows do not appear. */
    cbm_node_t *nodes = NULL;
    int nc = 0;
    ASSERT_EQ(cbm_store_find_nodes_by_label(s, "test::missed", "File", &nodes, &nc), CBM_STORE_OK);
    ASSERT_EQ(nc, 2);
    int saw_partial_file = 0;
    int saw_oversized_file = 0;
    for (int i = 0; i < nc; i++) {
        if (strcmp(nodes[i].file_path, "src/a.py") == 0) {
            ASSERT_NOT_NULL(strstr(nodes[i].properties_json, "\"kind\":\"parse_partial\""));
            ASSERT_NOT_NULL(strstr(nodes[i].properties_json, "\"detail\":\"4-7\""));
            saw_partial_file++;
        } else if (strcmp(nodes[i].file_path, "gone.py") == 0) {
            ASSERT_NOT_NULL(strstr(nodes[i].properties_json, "\"kind\":\"oversized\""));
            ASSERT_NOT_NULL(strstr(nodes[i].properties_json, "\"detail\":\"too big\""));
            saw_oversized_file++;
        }
    }
    ASSERT_EQ(saw_partial_file, 1);
    ASSERT_EQ(saw_oversized_file, 1);
...
SUITE(store_nodes) {
    RUN_TEST(store_coverage_roundtrip_shadow);
