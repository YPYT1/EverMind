...
static inline int th_project_db_path(char *buf, size_t buf_size, const char *project) {
    const char *cache_dir = cbm_resolve_cache_dir();
    if (!buf || buf_size == 0 || !project || !cache_dir || !cbm_mkdir_p(cache_dir, 0755)) {
        return -1;
    }
    int written = snprintf(buf, buf_size, "%s/%s.db", cache_dir, project);
