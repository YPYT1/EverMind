...
.PARAMETER Target
    ...
    drive-picker guard's embedded HTTP UI) or 'cbm'. Guards that cannot run are
    failures because an official source bundle must include their prerequisites.
...
function Resolve-Binary {
    ...
    $makeArgs = @("-j", "-f", "Makefile.cbm", $Target, "SANITIZE=", "TMP=$tmp", "TEMP=$tmp", "TMPDIR=$tmp")
    if ((Get-Command gcc -ErrorAction SilentlyContinue) -and (Get-Command g++ -ErrorAction SilentlyContinue)) {
        ...
        throw "gcc/g++ or clang/clang++ is required to build the Windows test binary"
    }
    & $Make @makeArgs
...
# UI (build target cbm-with-ui). Any nonzero exit is a failed product guard.
$guards = @(
...
foreach ($t in $guards) {
    ...
    if (-not (Test-Path -LiteralPath $t)) {
        Write-Host "MISSING ($t) - required guard file is absent" -ForegroundColor Red
        $guardFailures += $t
        continue
    }
    & $py $t $bin
    ...
    if ($code -eq 0) {
        ...
        Write-Host "RED ($t) - unexpected test exit $code" -ForegroundColor Red
        $guardFailures += $t
    }
}
...
if (-not $GuardsOnly) {
    ...
    foreach ($t in $knownReds) {
        Write-Host "`n=== $t ===" -ForegroundColor Cyan
        if (-not (Test-Path -LiteralPath $t)) {
            Write-Host "MISSING ($t) - required repro file is absent" -ForegroundColor Red
            $guardFailures += $t
            continue
        }
        & $py $t $bin
