...
Orchestrates between cloud_client and space_catalog_service.
...
_SOURCE_RECOVERY_PROBE_TOP_K = config.EVERMIND_MCP_SOURCE_RECOVERY_PROBE_TOP_K
_SOURCE_RECOVERY_PROBE_CONCURRENCY = config.EVERMIND_MCP_SOURCE_RECOVERY_PROBE_CONCURRENCY
...
class MemoryService:
    def __init__(
        self,
        client: EverMindCloudClient,
        catalog: SpaceCatalogService,
        archive: ArchiveBridge | None = None,
    ):
        self._client = client
        self._catalog = catalog
        self._archive = archive or ArchiveBridge()
    ...
    def _validate_space_id(space_id: str) -> str:
        if not isinstance(space_id, str) or not space_id.strip():
            raise EverMindMCPError("space_id is required", code="INVALID_INPUT")
    ...
    def _uses_everos_backend() -> bool:
        return config.EVERMIND_MCP_BACKEND == "everos"
    ...
    @staticmethod
    def _is_profile_unsupported_search_error(error: EverMindMCPError) -> bool:
    ...
    @staticmethod
    def _is_user_scope_compat_error(error: EverMindMCPError) -> bool:
        ...
        *,
        ...
        if self._uses_everos_backend():
            ...
            processing_hint = (
                ...
                "Extraction timing depends on EverMind Cloud queue progress. "
    ...
    async def recall(
        ...
        *,
        ...
        if not successes:
            ...
            if isinstance(first_error, EverMindMCPError):
                raise first_error
            raise EverMindMCPError(
    ...
    async def briefing(
        ...
        *,
        ...
        if len(failures) == len(results):
            ...
            if isinstance(first_err, EverMindMCPError):
                raise first_err
            raise EverMindMCPError(
        ...
        if project_slug := self._archive.project_slug_from_space(space_id):
            output["archive"] = self._archive.read_project_summary(
                project_slug
            )

        return output

    # -- EverMind Archive candidate workflow --

    async def propose_archive_update(
        self,
        *,
        project_slug: str,
        target_file: str,
        ...
        return self._archive.propose_update(
    ...
    async def commit_archive_update(
        self,
        *,
        candidate_id: str,
        confirmed: bool = False,
    ) -> dict:
        return self._archive.commit_update(
    ...
    async def forget(
        ...
        *,
        ...
        async def _delete_one(
            mid: str,
        ) -> tuple[str, int, EverMindMCPError | None]:
            async with semaphore:
                ...
                except EverMindMCPError as e:
                    return mid, 0, e
                except Exception as e:  # pragma: no cover
                    return mid, 0, EverMindMCPError(
