"""HTTP adapter for EverMind Cloud API.
...
class EverMindMCPError(Exception):
    """Error from EverMind API interaction."""
...
class EverMindCloudClient:
    """Async HTTP client for EverMind Cloud v0 API.
    ...
    def __init__(
        ...
        self._base_url = config.EVERMIND_MCP_BASE_URL if base_url is None else base_url
        self._api_key = config.EVERMIND_MCP_API_KEY if api_key is None else api_key
        self._api_version = (
            config.EVERMIND_MCP_API_VERSION if api_version is None else api_version
        )
        self._user_id = config.EVERMIND_MCP_USER_ID if user_id is None else user_id
    ...
    async def __aenter__(self) -> "EverMindCloudClient":
    ...
    async def _handle(self, r: httpx.Response) -> dict:
        """Parse response; raise EverMindMCPError on 4xx/5xx."""
        if r.status_code >= 400:
            try:
                ...
                msg = r.text[:500]
                code = "UPSTREAM_ERROR"
            raise EverMindMCPError(msg, code=code, status_code=r.status_code)
    ...
    async def _request(self, method: str, path: str, **kwargs) -> dict:
        ...
        them as EverMindMCPError(code="UPSTREAM_UNAVAILABLE").
        """
    ...
    def _maybe_hint_get_body_stripping(
        error: EverMindMCPError, payload: dict
    ) -> EverMindMCPError:
        ...
        return EverMindMCPError(
            f"{msg} ({hint})", code=error.code, status_code=error.status_code
        )

    @staticmethod
    def _should_retry_delete_with_event_id(error: EverMindMCPError) -> bool:
    ...
    def _normalize_group_ids(
        ...
        *,
        ...
        if group_ids is None:
            if allow_none:
                return None
            raise EverMindMCPError(f"{field_name} is required", code="INVALID_INPUT")
    ...
    async def _request_get_with_json_fallback(self, path: str, payload: dict) -> dict:
        ...
        try:
            ...
            if hinted is not exc:
                ...
                except EverMindMCPError as post_error:
    ...
    async def search_memories(
        ...
        *,
        ...
        EverMind search endpoint uses **GET with JSON body**.
        """
        self._require_key()
    ...
    async def get_conversation_metadata(self, group_id: str) -> dict:
        ...
        if not isinstance(group_id, str) or not group_id.strip():
            raise EverMindMCPError("group_id is required", code="INVALID_INPUT")
