...
Writes are also persisted to a reserved EverMind space for cross-session recovery.
"""
...
def to_group_id(space_id: str) -> str:
    """Convert user-facing space_id to EverMind group_id."""
...
class SpaceCatalogService:
    """In-memory space registry backed by EverMind for persistence.
    ...
    def __init__(self, client: EverMindCloudClient):
        self._client = client
    ...
    async def register_space(
        ...
        *,
        ...
        """Register or update a space. Persists to EverMind (best-effort)."""
    ...
    async def get_conversation_meta(
        ...
        *,
        ...
        if not EVERMIND_MCP_ENABLE_CONVERSATION_META:
            return None
    ...
    def get_recovery_warning(self) -> dict | None:
        ...
        if self._recover_last_error.get("code") == "AuthenticationError":
            warning["hint"] = (
                ...
                "Verify the active EVERMIND_MCP_API_KEY in the runtime environment."
            )
        return warning
    ...
    @staticmethod
    def _is_group_scene_desc_compat_error(exc: EverMindMCPError) -> bool:
        message = str(exc).lower()
        return "scene_desc" in message and "group-level config" in message

    @staticmethod
    def _is_group_scene_inherited_error(exc: EverMindMCPError) -> bool:
        message = str(exc).lower()
        return "group-level config" in message and "cannot set 'scene'" in message

    @staticmethod
    def _requires_group_name_error(exc: EverMindMCPError) -> bool:
        message = str(exc).lower()
    ...
    async def _set_conversation_metadata_compat(
        ...
        *,
        ...
        payload = {
            ...
            "tags": tags,
            "llm_custom_setting": EVERMIND_MCP_LLM_CUSTOM_SETTING,
            "user_details": user_details,
            "default_timezone": EVERMIND_MCP_DEFAULT_TIMEZONE,
        }
    ...
    async def _persist_conversation_meta(
        ...
        *,
        ...
        scene = EVERMIND_MCP_CONVERSATION_SCENE
    ...
    def _merge_user_details(
        *,
        ...
        user_details = EVERMIND_MCP_USER_DETAILS
    ...
    async def _try_recover(self) -> None:
        """Best-effort: recover space list from EverMind catalog space.
        ...
        try:
            ...
            if self._cache and EVERMIND_MCP_ENABLE_CONVERSATION_META:
                ranked = sorted(
                    self._cache.values(),
