"""Basic Memory bridge for Chinese project knowledge files."""
...
from . import config
from .evermemos_client import EverMemosError
...
class BasicMemoryBridge:
    def __init__(
        ...
        *,
        ...
        self.root = Path(root or config.BASIC_MEMORY_ROOT)
        self.candidate_dir = Path(candidate_dir or config.BASIC_MEMORY_CANDIDATE_DIR)
        self.write_policy = (write_policy or config.BASIC_MEMORY_WRITE_POLICY).lower()
    ...
    def read_project_summary(self, project_slug: str, *, max_chars: int = 12000) -> dict:
        ...
        if not project_dir.exists():
            return {
                "available": False,
                "project_slug": slug,
                "message": f"Basic Memory project folder not found: {project_dir}",
            }
    ...
    def propose_update(
        ...
        *,
        ...
        if not isinstance(content, str) or not content.strip():
            raise EverMemosError("content is required", code="INVALID_INPUT")
        if not isinstance(evidence, str) or not evidence.strip():
            raise EverMemosError("evidence is required", code="INVALID_INPUT")
        if not isinstance(reason, str) or not reason.strip():
            raise EverMemosError("reason is required", code="INVALID_INPUT")
    ...
    def commit_update(self, *, candidate_id: str, confirmed: bool = False) -> dict:
        if not confirmed:
            raise EverMemosError(
                "confirmed=true is required before writing to Basic Memory",
                code="INVALID_INPUT",
            )
        if not isinstance(candidate_id, str) or not candidate_id.strip():
            raise EverMemosError("candidate_id is required", code="INVALID_INPUT")

        path = self.candidate_dir / f"{candidate_id.strip()}.json"
        if not path.exists():
            raise EverMemosError("candidate not found", code="NOT_FOUND")
        ...
        if not content or not evidence:
            raise EverMemosError("candidate is missing content/evidence", code="INVALID_INPUT")
    ...
    def _validate_project_slug(value: str) -> str:
        if not isinstance(value, str) or not value.strip():
            raise EverMemosError("project_slug is required", code="INVALID_INPUT")
        slug = value.strip().lower()
        if any(char in slug for char in "\\/:*?\"<>|") or slug in {".", ".."}:
            raise EverMemosError("project_slug contains invalid path characters", code="INVALID_INPUT")
        return slug

    @staticmethod
    def _validate_target_file(value: str) -> str:
        if not isinstance(value, str) or not value.strip():
            raise EverMemosError("target_file is required", code="INVALID_INPUT")
        filename = value.strip()
        if any(char in filename for char in "\\/:*?\"<>|") or filename in {".", ".."}:
            raise EverMemosError("target_file contains invalid path characters", code="INVALID_INPUT")
        if filename not in ALLOWED_TARGET_FILES and not (
            filename.startswith("模块-") and filename.endswith(".md")
        ):
            raise EverMemosError(
                "target_file must be a known Chinese Basic Memory file or 模块-*.md",
                code="INVALID_INPUT",
            )
        return filename
